...
public class GuiBaseEdgeRenderer extends EdgeRenderer {
	...
	/** Cache the graph */
	protected transient WeakReference<BasicGraph> _graph;

	/** Cache the edgeview */
	protected transient WeakReference<EdgeView> _view;

	protected transient Color defaultForeground, defaultBackground;

	protected transient boolean selected, focus, preview;
	
	protected transient Bezier _bezier;
	
	protected transient Spline2D _spline;
	
	/**
	 * Constructs a renderer that may be used to render edges.
	 */
	public GuiBaseEdgeRenderer(BasicGraph graph, GuiEdgeView edgeView) {
		...
		_view =  new WeakReference<EdgeView> (edgeView);
		installAttribs ();		
	}
	
	private void installAttribs ()
	{
		EdgeView view = _view.get();
		if (view != null)
		{
			Color foreground = GraphConstants.getLineColor(view.getAllAttributes());
			...
			Color gradientColor = GraphConstants.getGradientColor(view.getAllAttributes());
			setGradientColor(gradientColor);
			setOpaque(GraphConstants.isOpaque(view.getAllAttributes()));
	 ...
	 */
	public Component getRendererComponent(final JGraph graph, final CellView view,
			final boolean sel, final boolean focus, final boolean preview) {
		if (view == _view.get() && _view.get()!=null) {
			this.focus = focus;
			this.selected = sel;
	 ...
	 */
	public boolean intersects(final JGraph graph, 
							  final CellView value, 
							  final Rectangle rect) {
		EdgeView edgeView = _view.get();
		if (edgeView == _view.get() && edgeView!=null) {
			...
			if (edgeView.getPointCount() == 2) {
				...
				if (g2.hit(rect, edgeView.getShape(), true))
					return true;					
			}
			Rectangle2D r = getLabelBounds(graph, edgeView);
			if (r != null && r.intersects(rect))
				return true;
			Object[] labels = GraphConstants.getExtraLabels(edgeView
					.getAllAttributes());
			if (labels != null) {
				for (int i = 0; i < labels.length; i++) {
					r = getExtraLabelBounds(graph, edgeView, i);
					if (r != null && r.intersects(rect))
						return true;
				}
			}
	...
	public Rectangle2D getBounds(CellView value) {
		...
		if (value == view && view!=null) {
			...
			int b = (int) Math.ceil(getLineWidth());
	...
	private boolean isLabelTransformEnabled() {
		EdgeView view = _view.get();
		if (view!=null)
			return GraphConstants.isLabelAlongEdge(view.getAllAttributes());
		else
			return false;
	}
	...
	public Rectangle2D getLabelBounds(JGraph paintingContext, EdgeView view) {
		if (paintingContext == null && _graph.get() != null) {
			JGraph graph = _graph.get();
	...
	public Rectangle2D getExtraLabelBounds(JGraph paintingContext,
		...
		EdgeView edgeView = _view.get();
		if (labels != null && index < labels.length && edgeView!=null) {
			Point2D p = getExtraLabelPosition(edgeView, index);
			Dimension d = getExtraLabelSize(paintingContext, edgeView, index);
			String label = (paintingContext != null) ? paintingContext
	...
	public Point2D getLabelPosition(EdgeView view) {
		EdgeView edgeView = _view.get(); 
		if (view == edgeView && edgeView!=null)
	...
	public void paint(Graphics g) {
		...
		if (view!=null && graph!=null)
		{
			...
			final float lineWidth = GraphConstants.getLineWidth(view.getAllAttributes());
			final Color gradientColor = GraphConstants.getGradientColor(view.getAllAttributes());
			final int beginDeco = GraphConstants.getLineBegin(view.getAllAttributes());
			final boolean beginFill = GraphConstants.isBeginFill(view.getAllAttributes()) && isFillable(beginDeco);
			final int endDeco = GraphConstants.getLineEnd(view.getAllAttributes());
			final boolean endFill = GraphConstants.isEndFill(view.getAllAttributes()) && isFillable(endDeco);
			final float[] lineDash = GraphConstants.getDashPattern(view.getAllAttributes());
			final float dashOffset = GraphConstants.getDashOffset(view.getAllAttributes());
	...
	protected void paintSelectionBorder(Graphics g) {
		...
		if (graph!=null && view != null)
		{
			...
			if (graph.getSelectionModel()
					.isChildrenSelected(view.getCell()) || selected) {
				Dimension d = getSize();
	...
	protected void paintLabel(Graphics g, String label, Point2D p,
		...
		if (graph!=null && view != null)
		{
			if (p != null && label != null && label.length() > 0 && metrics != null) {
				...
				Color borderColor = GraphConstants.getBorderColor(view.getAllAttributes());
				...
				int dy = +sh / 4;
				Color tmp = GraphConstants.getForeground(view.getAllAttributes());
				Color fontColor = (tmp != null) ? tmp : getForeground();
		...
		if (view!=null)
		{
			synchronized (view) { 
				...
				if (n > 1) {
					...
					Edge.Routing routing = GraphConstants.getRouting(view.getAllAttributes());
					int lineStyle = (routing != null && view instanceof EdgeView) ? routing
							.getPreferredLineStyle((EdgeView) view)
							: Edge.Routing.NO_PREFERENCE;
					if (lineStyle == Edge.Routing.NO_PREFERENCE)
						lineStyle = GraphConstants.getLineStyle(view.getAllAttributes());
	
					final int beginDeco = GraphConstants.getLineBegin(view.getAllAttributes());
					final int beginSize = GraphConstants.getBeginSize(view.getAllAttributes());
					final int endDeco = GraphConstants.getLineEnd(view.getAllAttributes());
					final int endSize = GraphConstants.getEndSize(view.getAllAttributes());
					
					
					if (lineStyle == GraphConstants.STYLE_BEZIER && n > 2) {
						_bezier = new Bezier(p);
						p2 = _bezier.getPoint(_bezier.getPointCount() - 1);
					} else if (lineStyle == GraphConstants.STYLE_SPLINE && n > 2) {
						_spline = new Spline2D(p);
						double[] point = _spline.getPoint(0.9875);
					...
					{
						System.out.println (view.sharedPath);
						throw ex;
					}
					/* THIS CODE WAS ADDED BY MARTIN KRUEGER 10/20/2003 */
					if (lineStyle == GraphConstants.STYLE_BEZIER && n > 2) {
						Point2D[] b = _bezier.getPoints();
	...
	private float getLineWidth ()
	{
		EdgeView view = _view.get();
		if (view!=null)
			return GuiGraphConstants.getLineWidth(view.getAttributes());
		else
			return 0F;
	}
