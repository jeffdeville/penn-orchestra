...
public class RuleSqlGen implements IRuleCodeGen {
	...
	protected Map<String,ISqlExpression> _whereExpressions;
	protected Set<ISqlExpression> _whereRoots;
...
protected List<ISqlFromItem> buildFrom() {
	...
	for (Atom a : m_rule.getBody()) {
		...
		else if(UDFunctions.isUDF(a.getRelation().getName()))
		{
			...
			ISqlExpression expr = _whereExpressions.get(a.getValues().get(0).toString());
...
private ISqlExpression getWhereRootContaining(ISqlExpression expr){
	for(ISqlExpression s: _whereRoots){
...
protected ISqlExpression conditionsForConstant(RelationField attribute, AtomConst c, 
...
protected ISqlExpression conditionsForVariable(RelationField attribute, AtomVariable var,
...
protected ISqlExpression buildWhereForAtom(Atom a, int i, Map<String,String> map, ISqlExpression expr, int curIterCnt){
...
//		}
		...
		ISqlExpression stratumExpr = stratumCondition(a, i, curIterCnt);
...
//			is not a special OU attribute
			...
			{
				ISqlExpression constConditions = conditionsForConstant(attribute, (AtomConst)atomVal, i, comparable, compareNullCol);

				expr = conjoin(expr, constConditions);
			} 
			else if (atomVal instanceof AtomVariable) 
			{
				ISqlExpression varConditions = conditionsForVariable(attribute, (AtomVariable)atomVal, i, comparable && !isSpecialAttrib, compareNullCol, map);
...
protected ISqlExpression buildWhere(int curIterCnt) {
	ISqlExpression expr = null;
...
//				SqlExpression newExpr = OLDbuildWhereForAtom(a, i, m_varmap, null, curIterCnt);
				ISqlExpression newExpr = buildWhereForAtom(a, i, m_varmap, null, curIterCnt);
...
//				}
	...
	Iterator<ISqlExpression> whereExps = _whereRoots.iterator();

	while (whereExps.hasNext()) {
		ISqlExpression nx = whereExps.next();
...
protected ISqlExpression conjoin(ISqlExpression expr, ISqlExpression cond) {
	if (expr == null) {
		return cond;
	} else if (cond == null) {
		return expr; 
