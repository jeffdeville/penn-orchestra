...
public class SchemaConverterStatementsGen {
	...
	// public final static String ALTER_NO_LOGGING =
	// " ACTIVATE NOT LOGGED INITIALLY";

	// TODO: Would be better to keep indexes tablespaces but is it possible via
	// JDBC ??
	// TODO: It also loses deferrability and this kind of qttributes for foreign
	// keys
	...
	public SchemaConverterStatementsGen(DataSource ds, String jdbcDriver,
			Schema sc) {
		_ds = ds;
		_sc = sc;
	...
	/**
	 * Create the SQL statements necessary to fill in any missing tables in the
	 * schema
	 * 
	 * @param statements The SQL statements will be added in this list. Order
	 *            must be respected for execution
	 * @return List of relations
	 */
	public List<String> createNecessaryTables(boolean withNoLogging,
			String database, boolean stratified, boolean withLogging, SqlDb db) {
	 ...
	 * @return List of relations that already contain labeled null columns but
	 *         without the correct characteristics
	 */
	public Map<Relation, List<String>> createTableConversionStatements(
			List<String> statements, boolean withNoLogging, String database,
			boolean stratified, String NO_LOGGING, SqlDb db,
		...
		createDeltaTables(statements, withNoLogging, database, stratified,
				NO_LOGGING, db, containsBidirectionalMappings);
	...
	private void OLDcreateDeltaTables(List<String> statements,
			boolean withNoLogging, String database, boolean stratified,
			String NO_LOGGING, SqlDb db) {
		...
		for (Relation rel : _sc.getRelations()) {
			for (AtomType type : AtomType.values()) {
				...
				// if((relName.endsWith("_L") || relName.endsWith("_R")) &&
				// (type == AtomType.RCH)){
				if (rel.isInternalRelation() && (type == AtomType.RCH)) {
					...
					if (type != AtomType.NONE) {
						if (_existingTableNames.contains(rel
								.getFullQualifiedDbId()
								+ "_" + type.toString()))
							statements.add("DROP TABLE "
									+ rel.getFullQualifiedDbId() + "_"
									+ type.toString());
					}

					// String tablename = rel.getFullQualifiedDbId() + "_" +
					// type.toString();
					String tablename = rel.getFullQualifiedDbId();

					// String stratum = "";
					// if(!tablename.endsWith("_L") && !tablename.endsWith("_R")
					// && (type == AtomType.DEL)){
					...
					if (stratified
						...
						// SqlColumnDef s = new SqlColumnDef("STRATUM",
						// "INTEGER", "0");
					...
					if (type != AtomType.NONE) {
						...
						String stat = "CREATE " + typ + "TABLE " + tablename
								+ "(" + stratum;
						...
						for (int i = 0; i < rel.getFields().size(); i++) {
							...
							cols.add(_sqlFactory.newColumnDef(rel.getField(i)
									.getName(), rel.getField(i)
									.getSQLTypeName(), null));
						}
						for (int i = 0; i < rel.getFields().size(); i++) {
							stat = stat + "," + rel.getField(i).getName()
									...
									+ " INTEGER";// +
							// rel.getField(i).getDbType();

							if (tablename.endsWith("_L_INS")
									|| tablename.endsWith("_R_INS")
									|| tablename.endsWith("_L_DEL")
									|| tablename.endsWith("_R_DEL"))
								cols.add(_sqlFactory.newColumnDef(rel.getField(
										i).getName()
										+ RelationField.LABELED_NULL_EXT,
										"INTEGER", "1"));
							else
						 ...
						 * + ")"; if (_jdbcDriver.contains("db2")) { statement =
						 * statement + " WITH NO DATA";
						 * 
						 * if (!withLogging) statement = statement + NO_LOGGING;
						 * }
						 */
						// if (withNoLogging)
						...
						// statements.add(stat);
						List<String> stmts = db.getSqlTranslator().createTable(
								tablename, cols, withNoLogging);
					...
					if (tablename.endsWith("_L_INS")
						...
						for (RelationField fld : rel.getFields()) {
							...
							// boolean fldInKey =
							// rel.getPrimaryKey().getFields().contains(fld);
					...
					if (stratified) {
						if (tablename.endsWith("_L_DEL")
								|| tablename.endsWith("_R_DEL")
								|| tablename.endsWith("_L_INS")
								|| tablename.endsWith("_R_INS")) {
							StringBuffer buf = new StringBuffer();
							buf.append("ALTER TABLE " + tablename);
							if (database.contains("db2"))
								buf
										.append(" ALTER COLUMN STRATUM SET DEFAULT 0");
							else
					...
					boolean first = true;
					for (RelationField fld : rel.getFields()) {
						boolean fldInKey = rel.getPrimaryKey().getFields()
								.contains(fld);
						if (fldInKey) {
							if (first) {
								first = false;
	...
	private void createDeltaTables(List<String> statements,
			boolean withNoLogging, String database, boolean stratified,
			String NO_LOGGING, SqlDb db, boolean containsBidirectionalMappings) {
		...
		for (Relation rel : _sc.getRelations()) {
			for (AtomType type : AtomType.values()) {
				...
				// if ((rel.getName().endsWith(Relation.LOCAL) && (type ==
				// AtomType.RCH)) ||
				// (rel.getName().endsWith(Relation.REJECT) && (type !=
				// AtomType.NONE)))
				{
					// We don't need to create _RCH tables for idbs
				} else if (!stratified || type != AtomType.ALLDEL) {
					...
					// && type != AtomType.NONE
					// ){
					// tablename = SqlStatementGen.sessionSchema + "." +
					// rel.getDbRelName();
					// } else {
					...
					if (stratified
						...
						if (tablename.endsWith("_L_DEL")
							...
							cols.add(_sqlFactory.newColumnDef("STRATUM",
									"INTEGER", "0"));
						} else {
							cols.add(_sqlFactory.newColumnDef("STRATUM",
									"INTEGER", null));
					...
					if (type != AtomType.NONE) {
												...
												.add(_sqlFactory
														.newColumnDef(
																rel
																		...
																		.getName()
																		+ RelationField.LABELED_NULL_EXT,
																"INTEGER", Integer.toString(SqlEngine.LABELED_NULL_NONVALUE)));
									else
										cols
												.add(_sqlFactory
														.newColumnDef(
						...
						if (!Config.getTempTables()
								|| !(tablename
										.startsWith(ISqlStatementGen.sessionSchema) // ||
								// type
								// !=
								// AtomType.NONE
								)) {
							System.out.println("* Adding non-temp table "
									+ tablename);
							statements
									.addAll(db.getSqlTranslator().createTable(
											tablename, cols, withNoLogging));
						} else {
							System.out.println("* Adding delta temp table "
									+ tablename);
							statements.addAll(db.getSqlTranslator()
									.createTempTable(tablename, cols));
						}
					...
					for (RelationField fld : rel.getFields()) {
						boolean fldInKey = rel.getPrimaryKey().getFields()
								.contains(fld);
						if (fldInKey) {
							v.add(_sqlFactory.newColumnDef(fld.getName(), "",
									null));
						}
					}
					...
					if (!INDEX_ALL_FIELDS) {
						...
						if (rel.hasLabeledNulls()) {
							...
							for (RelationField fld : rel.getFields()) {
								if (!Config.useCompactNulls()
										|| rel.isNullable(inx))
									v.add(_sqlFactory.newColumnDef(fld
											.getName()
											+ RelationField.LABELED_NULL_EXT,
											"", null));
					...
					String stmt;
					// gregkar
					stmt = db.getSqlTranslator().createIndex(
							tablename + "_IDX ", tablename, v, true, true);
	...
	/**
	 * First step is to remove constraints that will be affected by skolems
	 * introduction. They will be recreated later
	 * 
	...
	private void removeConstraints(List<String> statements) {
		for (Relation rel : _sc.getRelations()) {
			...
			/*
			 * Let's keep the non unique indexes. skolem nulls are not
			 * discriminant enough
			 */
			/*
	...
	/**
	 * For each table, add a labeled null column for each column that does not
	 * have one already
	 * 
	 * @param statements List to which statements must be added
	 * @return For each relation having "errors": list of labeled null fields
	 *         already defined but incorrects
	 */
	private Map<Relation, List<String>> addLabeledNullColumns(
			List<String> statements, boolean withNoLogging, SqlDb db)
			throws MetaDataAccessException {
		Map<Relation, List<String>> tabErrors = new HashMap<Relation, List<String>>();
		for (Relation rel : _sc.getRelations()) {
			if (rel.hasLabeledNulls()) {
				List<String> errors = addLabeledNullColumns(rel, statements,
						withNoLogging, db);
				if (errors.size() > 0)
	...
	/**
	 * For each column in the relation, add a new column to contain the labeled
	 * null id.
	 * 
	 * @param rel Relation for which labeled null columns must be added
	...
	private List<String> addLabeledNullColumns(Relation rel,
		...
		MetaLoadColumns loader = new MetaLoadColumns(rel.getDbCatalog(), rel
				.getDbSchema(), rel.getDbRelName(), labNulls);
		List<String> errors = null;
		// try
		// {
		errors = (List<String>) JdbcUtils.extractDatabaseMetaData(_ds, loader);
		/*
		 * } catch (MetaDataAccessException ex) { //TODO: Actually deal with
		 * exceptions here! assert (errors!=null) : "not implemented yet"; }
		 */
		...
		for (RelationField fld : rel.getFields()) {
			...
			if ((!Config.useCompactNulls() || rel.isNullable(inx))
					&& !labNulls.contains(fld.getName()
							+ RelationField.LABELED_NULL_EXT))
				cols.add(_sqlFactory.newColumnDef(fld.getName()
						+ RelationField.LABELED_NULL_EXT, "INTEGER", Integer
						.toString(SqlEngine.LABELED_NULL_NONVALUE)));
			// statements.add ("ALTER TABLE " + rel.getFullQualifiedDbId()
			// + " ADD " + fld.getName() + ModelConstants.LABEL_NULL_EXT
			// //+ " NUMERIC (2, 0) WITH DEFAULT 1" + extra
			// + " INTEGER DEFAULT 1" + extra
			// //TODO: Fix pb: if create table as select it doesn't copy default
			// value!!!
			// /*+ " DEFAULT -1"
			// + " NOT NULL"*/);
			// // }
		...
		statements.addAll(db.getSqlTranslator().addColsToTable(
				rel.getFullQualifiedDbId(), cols, withNoLogging));
		return errors;
	}

	/**
	 * Recreate all the constraints, including the newly created labeled null
	 * columns
	 * 
	...
	/**
	 * Recreate the relation primary key (if any), including the new labeled
	 * null columns
	 * 
	...
	private void createPrimaryKey(Relation rel, List<String> statements) {
		...
		if (pk != null) {
			...
			statements.add("ALTER TABLE " + rel.getFullQualifiedDbId()
					+ " ADD CONSTRAINT " + pk.getName() + " PRIMARY KEY "
					+ fieldsList);
		}
	}

	/**
	 * Recreate the relation unique indexes including the new labeled null
	 * columns
	 * 
	...
	private void createUniqueIndexes(Relation rel, List<String> statements) {
		for (RelationIndexUnique idx : rel.getUniqueIndexes()) {
			...
			statements.add("CREATE UNIQUE INDEX " + idx.getName() + " ON "
					+ rel.getFullQualifiedDbId() + fieldsList + "CLUSTER");
			// statements.add("ALTER TABLE " + idx.getName() +
			// " VOLATILE CARDINALITY");
		}
	}

	/**
	 * Recreate the relation foreign keys including the new labeled null columns
	 * 
	...
	private void createForeignKeys(Relation rel, List<String> statements) {
		for (ForeignKey fk : rel.getForeignKeys()) {
			...
			String refFieldsLString = getCommaSepFieldsList(rel, fk
					.getRefFields());
			statements.add("ALTER TABLE " + rel.getFullQualifiedDbId()
					+ " ADD CONSTRAINT " + fk.getName() + " FOREIGN KEY "
					+ fkFieldsList + " REFERENCES "
					+ fk.getRefRelation().getName() + refFieldsLString);
		}
	}
	 ...
	 * 
	 * @param fields Fields to include in the list, should not contain labeled
	 *            null fields
	 * @return comma-separated list of fields names
	 */
	private String getCommaSepFieldsList(Relation rel,
		...
		for (RelationField fld : fields) {
			...
			buff.append(fld.getName());
			if (rel.hasLabeledNulls()
					&& (!Config.useCompactNulls() || rel.isNullable(inx))) {
				buff.append(", ");
	...
	/**
	 * Metadata callback class used to load a table's columns and store those
	 * information in order to know what tables/columns are already prepared for
	 * labeled nulls Based on the Spring framework / JDBC module
	 * 
	...
	private class MetaLoadColumns implements DatabaseMetaDataCallback {
		...
		public MetaLoadColumns(String catalog, String schema, String tableName,
				List<String> fields) {
			_catalog = catalog;
			_schema = schema;
		...
		/**
		 * @return If there are incorrect labeled null fields already defined in
		 *         the database: list of those fields names
		 */
		public Object processMetaData(DatabaseMetaData dbmd)
			...
			while (rs.next()) {
				// Read column def first because of a bug in Oracle (column is
				// stored as a long and has to be read first!)
				...
				if (colName.endsWith(RelationField.LABELED_NULL_EXT)) {
					if ((rs.getInt("DATA_TYPE") != Types.NUMERIC && rs
							.getInt("DATA_TYPE") != Types.DECIMAL)
