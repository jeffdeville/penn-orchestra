 ...
 * 
 */
public class OrchestraOperationExecutor implements IOperationExecutor {

	/** The logger. */
	private final Logger logger = LoggerFactory.getLogger(getClass());
	
	/**
	 * The directory where the operation files are located. Also the location of
	 * any associated data files (for import).
	 */
	private final File testDataDirectory;

	/** The DbUnit tester for dealing with datasets. */
	 ...
	 * @param factory
	 */
	public OrchestraOperationExecutor(IOrchestraOperationFactory factory) {
		this.testDataDirectory = factory.getTestDataDirectory();
		this.dbTester = factory.getJdbcDatabaseTester();
	...
	public final void execute() throws Exception {
		File[] operations = testDataDirectory.listFiles(new FileFilter() {
			// Expected filename format: <number>-<operation name>[-<optional
			// peer name>].xml
		...
		for (String fileName : fileNames) {
			...
			String[] parsedName = fileName.replace(".xml", "").split("-", 2);
			...
			if (operation.equalsIgnoreCase("insert")) {
				...
				opFactory.newOperation(operation,
						datasetFile).execute();
			}
		}
	}
