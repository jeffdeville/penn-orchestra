...
public class GuiVertexRenderer extends VertexRenderer {
	...
	public Point2D getPerimeterPoint(VertexView view, Point2D source, Point2D p) {
		int shape = GuiGraphConstants.getVertexShape(view
				.getAllAttributes());
		if (shape == SHAPE_DIAMOND) {
	...
	protected void resetAttributes() {
		...
		shape = GuiGraphConstants.getVertexShape(view.getAllAttributes());
		isRichText = false;
		valueComponent = null;
	}
	...
	public void installAttributes(CellView view) {
		...
		shape = GuiGraphConstants.getVertexShape(view.getAllAttributes());
		stretchImage = GuiGraphConstants.isStretchImage(map);
